...
def get_pymc_model(params, model, batch_size):
    ...
    with ml.pymc3.InitializableModel(setup=setup) as pymc_model:
        ...
        # TODO: Use shared variables, that way the `logp` function will work
...
def get_sgd(params, model, pymc_model,
    ...
    # HACK: comp. graphs for kernels need to be updated as well
    # FIXME: If they were evaluated when needed (rather than in
    #        `discretize_θkernel`), the subs. in model.params would suffice
    model.θ_dis._data = shim.graph.clone(
        shim.graph.clone(model.θ_dis._data, replace=var_subs),
        replace = sgd.var_subs)
    model.θtilde_dis._data = shim.graph.clone(
        shim.graph.clone(model.θtilde_dis._data, replace=var_subs),
        replace = sgd.var_subs)
