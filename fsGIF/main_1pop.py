...
def get_params():
    ...
    N = np.array((500,)) # No. of neurons in each pop
    p = np.array(((0.1009,),)) # Connection probs between pops
    ...
    model_params = gif.GIF_spiking.Parameters(
        N      = N,
        R      = np.array((1,)),     # Ω, membrane resistance; no value given (unit assumes I_ext in mA)
        u_rest = np.array((20.123,)),   # mV, p. 55
        p      = p,                    # Connection probability
        w      = ((0.176,),),    # mV, p. 55, L2/3
        Γ      = Γ,               # Binary connectivity matrix
        τ_m    = (0.02,),    # s,  membrane time constant
        #τ_m    = (0.02, 0.003),    # DEBUG
        t_ref  = (0.004,),  # s,  absolute refractory period
        u_th   = (15,),        # mV, non-adapting threshold  (p.54)
        u_r    = (0,),          # mV, reset potential   (p. 54)
        c      = (10,),        # Hz, escape rate at threshold
        Δu     = (5,),          # mV, noise level  (p. 54)
        Δ      = ((0.001,),),# s,  transmission delay
        τ_s    = ((0.003,),),# mV, synaptic time constants (kernel ε)
                                  # Exc: 3 ms, Inh: 6 ms
        # Adaptation parameters   (p.55)
        J_θ    = (1.0,),        # Integral of adaptation kernel θ (mV s)
        τ_θ    = (0.2,)
...
def get_fitmask(model, num):
    param_list = [model.params.w, model.params.τ_m, model.params.c,
                  model.params.u_rest, model.params.τ_s, model.params.u_th,
                  model.params.u_r, model.params.Δu]
    return {param: num == param_list.index(param) + 1
...
if __name__ == '__main__':
    ...
    #generate_activity(4)
    def f(datalen, seed):
        likelihood_sweep(('w', (0,)),
                         ('τ_m', (0,)),
                         fineness=(15,5),
                         burnin=0.5,
                         datalen=datalen,
                         input_filename = 'data/short_adap/spikes/fsgif_1pop_no-input_9s_{:0>3}seed'.format(seed),
                         output_filename = 'data/short_adap/likelihoods/fsgif_1pop_no-input_{}s_{}seed_loglikelihood_theano'.format(datalen, seed))
